...
interface LoginFormProps {
  ...
  activeSocialProvider?: 'google' | 'microsoft' | 'apple' | null;
}
    ...
    <div className="space-y-4 text-left">
      ...
      <div className="grid grid-cols-3 gap-2.5">
        ...
        <button
          ...
          className="w-full py-2.5 px-3 bg-white border border-stone-200 hover:border-stone-300 hover:bg-stone-50/80 rounded-xl flex items-center justify-center gap-2 transition-all shadow-xs cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {loading && activeSocialProvider === 'google' ? (
            <svg className="w-4 h-4 animate-spin text-emerald-700 shrink-0" fill="none" viewBox="0 0 24 24">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
              <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
            </svg>
          ) : (
          ...
          <span className="text-xs font-semibold text-stone-700">
            {loading && activeSocialProvider === 'google' ? 'Signing in...' : 'Google'}
          </span>
        </button>
        ...
        <button
          ...
          className="w-full py-2.5 px-3 bg-white border border-stone-200 hover:border-stone-300 hover:bg-stone-50/80 rounded-xl flex items-center justify-center gap-2 transition-all shadow-xs cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {loading && activeSocialProvider === 'microsoft' ? (
            <svg className="w-4 h-4 animate-spin text-emerald-700 shrink-0" fill="none" viewBox="0 0 24 24">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
              <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
            </svg>
          ...
          <span className="text-xs font-semibold text-stone-700">
            {loading && activeSocialProvider === 'microsoft' ? 'Signing in...' : 'Microsoft'}
          </span>
        </button>
        ...
        >
          ...
          <span className="text-xs font-semibold text-stone-700">
            {loading && activeSocialProvider === 'apple' ? 'Signing in...' : 'Apple'}
          </span>
        </button>
      </div>
